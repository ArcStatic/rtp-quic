...
for i in range(len(app_seqnums)):
  ...
  if ((end_stack_times.get(stack_seqnums[i]) != None) and (end_app_times.get(app_seqnums[i]) != None)):
    ...
    if (playback_diff > playback_diff_prev):
      ...
      print("pb_delay_counter: %d (%f frames)"  % (pb_delay_counter, pb_delay_counter/diff_per_frame))
...
plt.ylim(pb_offsets_array[graph_start_idx], pb_offsets_array[graph_end_idx] + 0.025)
...
plt.savefig('offsets-single-150ms.png')
