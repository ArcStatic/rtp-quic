...
usage() 
{
	printf("Usage: rtpdemo [switches] address port dest_port\n");
...
{
	printf("Received data (payload %d timestamp %06d size %d data %s) ", p->pt, p->ts, p->data_len, p->data);
...
rtp_event_handler(struct rtp *session, rtp_event *e) 
{
	...
	case RX_BYE:
		...
		break;
	case SOURCE_CREATED:
    	printf("Event: SOURCE_CREATED\n");
		printf("New source created, SSRC = 0x%08x\n", e->ssrc);
		break;
	case SOURCE_DELETED:
	    printf("Event: SOURCE_DELETED\n");
...
rxtx_loop(struct rtp* session, int send_enable) 
{
	...
	if (send_enable) {
		printf("Sending on %s port %d and listening to ", 
		        rtp_get_addr(session),
		        rtp_get_tx_port(session));
	} else {
		printf("Listening to ");
	}
	...
	for(round = 0; round < MAX_ROUNDS; round++) {
		...
		/* Send data packets */
		if (send_enable) {
    		sprintf(mulaw_buffer, "customdata_round%d", round);
			rtp_send_data(session, rtp_ts, MULAW_PAYLOAD, 
				      0, 0, 0,
				      //(char*)mulaw_buffer, MULAW_BYTES, 
				      mulaw_buffer, MULAW_BYTES, 
				      0, 0, 0);
	        printf("message sent on 0x%08x: %s\n", rtp_my_ssrc(session), mulaw_buffer);
		}

		/* Receive control and data packets */
		timeout.tv_sec  = 5;
		timeout.tv_usec = 5000;
		recv_check = rtp_recv(session, &timeout, rtp_ts);
		printf("%s", recv_check ? "" : "Timeout occurred.\n");
...
main(int argc, const char *argv[]) 
{
	...
	address	= argv[ac];
	port	= atoi(argv[ac + 1]);
	dest_port	= atoi(argv[ac + 2]);

	session = rtp_init(address,		/* Host/Group IP address */ 
			   port,		/* receive port */
			   dest_port,		/* transmit port */
			   16,			/* time-to-live */
			   64000,		/* B/W estimate */
