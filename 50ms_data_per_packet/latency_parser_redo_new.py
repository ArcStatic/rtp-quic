...
for i in range(len(app_seqnums)):
  ...
  if ((end_stack_times.get(stack_seqnums[i]) != None) and (end_app_times.get(app_seqnums[i]) != None)):
    ...
    playback_offsets_array.append((end_app_times.get(app_seqnums[i]) - (end_stack_times.get(stack_seqnums[i])))/1000000.0)
...
plt.step(x, y, label='Delay between frames being received and passed to the application')
...
#plt.show()
#plt.savefig('app_latency/%s.pdf' % sys.argv[1][:-4])
plt.savefig('offsets-%s.pdf' % sys.argv[1][:-4])
