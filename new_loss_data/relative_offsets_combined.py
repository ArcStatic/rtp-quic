...
client_files = ["/home/vivian/Documents/L5project/rtp-quic/new_loss_data/RELIABLE_3B_0L_18000F_2000D_60R_client.txt", "/home/vivian/Documents/L5project/rtp-quic/new_loss_data/RELIABLE_3B_001L_18000F_2000D_60R_client.txt", "/home/vivian/Documents/L5project/rtp-quic/new_loss_data/RELIABLE_3B_0001L_18000F_2000D_60R_client.txt", "/home/vivian/Documents/L5project/rtp-quic/new_loss_data/RELIABLE_3B_00001L_18000F_2000D_60R_client.txt", "/home/vivian/Documents/L5project/rtp-quic/new_loss_data/RELIABLE_3B_03L_18000F_2000D_60R_client.txt"]


server_files = ["/home/vivian/Documents/L5project/rtp-quic/new_loss_data/RELIABLE_3B_0L_18000F_2000D_60R_server.txt", "/home/vivian/Documents/L5project/rtp-quic/new_loss_data/RELIABLE_3B_001L_18000F_2000D_60R_server.txt", "/home/vivian/Documents/L5project/rtp-quic/new_loss_data/RELIABLE_3B_0001L_18000F_2000D_60R_server.txt", "/home/vivian/Documents/L5project/rtp-quic/new_loss_data/RELIABLE_3B_00001L_18000F_2000D_60R_server.txt", "/home/vivian/Documents/L5project/rtp-quic/new_loss_data/RELIABLE_3B_03L_18000F_2000D_60R_server.txt"]
...
for f in range(len(server_files)):
  ...
  for i in range(len(app_seqnums)):
    ...
    if ((end_stack_times.get(stack_seqnums[i]) != None) and (end_app_times.get(app_seqnums[i]) != None)):
      ...
      print("sent time - played_time = playback_offset_time")
      ...
      #played_times.append(played_time/1000000000.0)
      ...
      sent_time = (start_app_times.get(app_seqnums[i]))
      #played_time = (end_app_times.get(app_seqnums[i]))
      played_time = (end_app_times.get(app_seqnums[i]) + (((pb_delay_counter)/3000) * diff_per_frame_ns))
      ...
      #played_times.append(played_time/1000000000.0)
      #ie. if it's an I-frame, playback time will be the same for all fragments
      #print("rtp_ts: %d" % playback_times_rtp_ts[i])
      if((playback_times_rtp_ts[i] - 3000) % 30000 == 0):
        #print("%d - %d = %d" % (played_time, first_ts, played_time - first_ts))
        #print("%f - %f = %f" % (played_time/1000000000.0, first_ts/1000000000.0, (played_time - first_ts)/1000000000.0))
        if (i == 0):
          played_times.append((played_time - first_ts)/1000000000.0)
        else:
          if ((((played_time - first_ts)/1000000000.0) - played_times[i-1]) < diff_per_frame):
            played_times.append(played_times[i-1] + diff_per_frame)
          else:
            played_times.append((played_time - first_ts)/1000000000.0 + diff_per_frame)
        
        print("played_times[%d]: %f" % (i, played_times[i]))
        print("sent_times[%d]: %f" % (i, sent_times[i]))
        #print("played_times appended val (i-frame): %f" % ((played_time - first_ts)/1000000000.0))
        #for i in range(fragment):
          #print("I-frame fragment updated: %f -> %f" % (played_times[i-fragment], played_times[i]))
          #played_times[i-fragment] = played_times[i]
          #print("I-frame fragment updated")
        fragment += 1
      #otherwise, it's a P-frame
        ...
        if(i == 0):
          ...
          print("Appended (initial): %f - %f = %f" % (played_times[i], sent_times[i], played_times[i] - sent_times[i]))
          print(playback_offsets_array[i])
        else:
          #print("frame pb check: %f - %f = %f" % ((played_time - first_ts)/1000000000.0, played_times[i-1], ((played_time - first_ts) - played_times[i-1])))
          #print("more than " + str(diff_per_frame) + "? " + str((((played_time - first_ts) - played_times[i-1]) < diff_per_frame)))
          if (fragment != 0):
            #append most recent I-frame playback time
            for j in range(fragment):
              #playback_offsets_array.append((played_times[i-fragment] - sent_times[i-fragment]))
              #print("Appended: %f - %f = %f" % (played_times[i-fragment], sent_times[i-fragment], played_times[i-fragment] - sent_times[i-fragment]))
              #playback_offsets_array.append((played_times[i-1] - sent_times[i-1]))
              #print("Appended (i-frame): %f - %f = %f" % (played_times[i-1], sent_times[i-1], played_times[i-1] - sent_times[i-1]))
              playback_offsets_array.append((played_times[i-fragment] - sent_times[i-fragment]))
              #playback_offsets_array.append((played_times[i-1] - sent_times[i-fragment]))
              #print("Appended (i-frame): %f - %f = %f" % (played_times[i-1], sent_times[i-fragment], played_times[i-1] - sent_times[i-1]))
              print("Appended (i-frame): %f - %f = %f" % (played_times[i-fragment], sent_times[i-fragment], played_times[i-fragment] - sent_times[i-fragment]))
              print("i-1 (%d - 1) = %d" % (i, i-1))
              print("i-fragment (%d - %d) = %d" % (i, fragment, i-fragment))
              print(playback_offsets_array[i-(fragment - j)])
            #playback_offsets_array.append((played_times[i-1] - (sent_time/1000000000.0)))
            #playback_offsets_array.append(played_times[i-1] - sent_times[i-1])
            #print(playback_offsets_array[i-1])
          if ((((played_time - first_ts)/1000000000.0) - played_times[i-1]) < diff_per_frame):
            #print("P-frame played as normal")
            #played_times.append((played_times[i-1] + diff_per_frame)/1000000000.0)
            played_times.append(played_times[i-1] + diff_per_frame)
            playback_offsets_array.append((played_times[i] - sent_times[i]))
            print("Appended (normal): %f - %f = %f" % (played_times[i], sent_times[i], played_times[i] - sent_times[i]))
            print(playback_offsets_array[i])
          else:
            #print("P-frame stall")
            played_times.append((played_time - first_ts)/1000000000.0)
            #playback_offsets_array.append((played_times[i] - (sent_time/1000000000.0)))
            playback_offsets_array.append(played_times[i] - sent_times[i])
            print("Appended (stall): %f - %f = %f" % (played_times[i], sent_times[i], played_times[i] - sent_times[i]))
            print(playback_offsets_array[i])
          fragment = 0
