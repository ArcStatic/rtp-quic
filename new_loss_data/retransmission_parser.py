...
stack_useful_packets = 0
stack_stale_packets = 0
...
for i in range(len(start_data)):
  ...
  if ("sent item" in start_data[i]):
    ...
    if (ssn in sent_seqnums):
      retransmissions += 1
    else:
      sent_seqnums.append(ssn)

packets_sent = len(sent_seqnums) + retransmissions
...
useful_packets = len(app_seqnums)
#should initially be 0 for reliable
stale_packets =  len(end_stack_times_array) - len(app_seqnums)
stalls = 0
...
print("Total packets sent: %d" % packets_sent)
...
#
#for partial
print("percentage of useful packets (sent): %f" % ((float(useful_packets)/packets_sent) * 100))
print("percentage of useful packets (received): %f" % ((float(useful_packets)/(stale_packets + useful_packets)) * 100))
print("percentage of stale packets (received): %f" % ((float(stale_packets)/(stale_packets + useful_packets)) * 100))
